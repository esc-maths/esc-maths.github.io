...
vec3 getPixel(in vec2 coord, float time) {  
    ...
    // uv = coord / iResolution.xy; // original uv definition
    // uv = uv * 2.0 - 1.0; // Not sure if I need this line
    //uv.x *= iResolution.x / iResolution.y; // Not sure if I need this line
        
    // ray
    vec3 ang = vec3(sin(time*0.3)*0.1,sin(time)*0.2+0.3,time*0.3);    
    vec3 ori = vec3(0.0,3.5,time*0.3);
...
void main() {
    ...
    float time = iTime * 0.3 + iMouse.x*0.01;
      ...
      for(int i = -1; i <= 1; i++) {
          for(int j = -1; j <= 1; j++) {
        	  vec2 uv = coord+vec2(i,j)/3.0;
    		  color += getPixel(uv, time);
          }
       }
      color /= 9.0;
    #else
      vec3 color = getPixel(coord, time);
    #endif
