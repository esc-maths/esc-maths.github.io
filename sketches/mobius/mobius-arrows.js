...
// --- Arrow Helper ---
// Returns 1.0 if inside a black arrow, 0.0 otherwise
float drawArrow(vec2 uv) {
    uv = uv * 2.0 - 1.0; // Center to -1, 1
    uv.y -= 0.1;         // Slight vertical offset for balance
    float arrow = 0.0;
    
    // Shaft
    float shaft = step(-0.1, uv.x) * step(uv.x, 0.1) * step(-0.5, uv.y) * step(uv.y, 0.4);
    // Head (Triangle)
    float head = step(0.0, uv.y - 0.2) * step(abs(uv.x), 1.0 - uv.y - 0.5);
...
void main() {
    ...
    if(t < 50.0) {
        ...
        vec2 gridUV;
        float angle = 0.0; // Used to rotate the arrow
        float speed = 0.07;

        if (uMode == 0) { // Elliptic (Rotation)
            gridUV = vec2(log(length(z) + 0.0001), atan(z.y, z.x) * (2.0 / PI) + iTime * speed * 2.2);
            angle = PI; // Flow is horizontal (along the checkerboard rows)
        } 
        else if (uMode == 1) { // Hyperbolic (Scaling)
            gridUV = vec2(log(length(z) + 0.0001) - iTime * speed * 2.4, atan(z.y, z.x) * (2.0 / PI));
            angle = PI * 0.5; // Flow is vertical (along the checkerboard columns)
        }
        else if (uMode == 2) { // Loxodromic (Spiral)
            float logR = log(length(z) + 0.0001) - iTime * speed;
            float theta = atan(z.y, z.x) * (2.0 / PI) + iTime * speed;
            float slope = 0.5; 
            gridUV.x = logR + slope * theta;
            gridUV.y = logR - (1.0 / slope) * theta; 
            gridUV *= 0.75;
            angle = 0.0; // Horizontal flow for loxodromic
        }
        else if (uMode == 3) { // Parabolic (Translation)
            gridUV = (z + vec2(iTime * speed * 2.2, 0.0)) * 1.0;
            angle = 3.0 * PI * 0.5; // Flow is horizontal but opposite direction (pointing left)
        }

        // --- Checkerboard & Arrows ---
        vec2 tileSpace = fract(gridUV * 4.0); // 0.0 to 1.0 within a tile
        vec2 check = floor(mod(gridUV * 4.0, 2.0));
        float isWhite = abs(check.x - check.y);

        // Rotate arrow based on flow angle
        mat2 rot = mat2(cos(angle), -sin(angle), sin(angle), cos(angle));
        vec2 arrowUV = (tileSpace - 0.5) * rot + 0.5;
        
        float arrowMask = drawArrow(arrowUV);
        
        // Only show arrow on white tiles
        float finalMask = isWhite;
        if(isWhite > 0.5) {
            finalMask = 1.0 - arrowMask; 
        }
        ...
        col = mix(vec3(0.01), vec3(0.9), finalMask) * diff;
