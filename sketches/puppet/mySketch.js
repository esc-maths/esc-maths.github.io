...
function setup() {
	...
	video.size(640, 480);  // Keep original camera resolution
	video.hide();
...
function movePuppet() {
	...
	if (hands.length > 0) {
		...
		// Scale the hand keypoints to match the larger video display
		let jointToTrack = hands[0].keypoints[12];
		let scaledX = jointToTrack.x * scaleFactor;
		let scaledY = jointToTrack.y * scaleFactor;
		head.moveTowards(scaledX, scaledY + yGap, 0.5);
		line(head.x, head.y, scaledX, scaledY);
		...
		scaledX = jointToTrack.x * scaleFactor;
		scaledY = jointToTrack.y * scaleFactor;
		leftLowerArm.moveTowards(scaledX, scaledY + yGap, 0.5);
		line(leftLowerArm.x, leftLowerArm.y, scaledX, scaledY);
		...
		scaledX = jointToTrack.x * scaleFactor;
		scaledY = jointToTrack.y * scaleFactor;
		rightLowerArm.moveTowards(scaledX, scaledY + yGap, 0.5);
		line(rightLowerArm.x, rightLowerArm.y, scaledX, scaledY);
		...
		scaledX = jointToTrack.x * scaleFactor;
		scaledY = jointToTrack.y * scaleFactor;
		leftKneeHingeB.moveTowards(scaledX, scaledY + yGap + 200 * pScale, 1);
		line(leftKneeHingeB.x, leftKneeHingeB.y, scaledX, scaledY);
		...
		scaledX = jointToTrack.x * scaleFactor;
		scaledY = jointToTrack.y * scaleFactor;
		rightKneeHingeB.moveTowards(scaledX, scaledY + yGap + 200 * pScale, 1);
		line(rightKneeHingeB.x, rightKneeHingeB.y, scaledX, scaledY);
	}
	pop();
}
...
function showVideo() {
	...
	// Draw video scaled up to fill the larger canvas
...
function showHandPoints() {
	...
	for (let i = 0; i < hands.length; i++) {
		...
		for (let j = 0; j < hand.keypoints.length; j++) {
			...
			text(j, keypoint.x * scaleFactor, keypoint.y * scaleFactor);
			 circle(keypoint.x * scaleFactor, keypoint.y * scaleFactor, 10);
		}
	}
}
...
function getHandedness() {
	for (let i = 0; i < hands.length; i++) {
		...
		let keypoint = hand.keypoints[ML5HAND_WRIST];
		// Scale the wrist position
		let wx = keypoint.x * scaleFactor;
		let wy = keypoint.y * scaleFactor;
		textSize(24);
		fill('lime');
...
function makePuppet() {
	...
	shoulders.x = xMax / 2;  // Now uses the larger xMax
	shoulders.y = yMax / 2;  // Now uses the larger yMax
	shoulders.color = 'cyan';
	...
	box = new Sprite([
		[1, 1],
		[xMax, 1],  // Uses the larger xMax
		[xMax, yMax],  // Uses the larger yMax
		[1, yMax],
		[1, 1]
	]);
	box.collider = "static";
