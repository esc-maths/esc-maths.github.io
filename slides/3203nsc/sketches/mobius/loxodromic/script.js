...
vec3 computeColor(vec2 fragCoord) {
    // 1. Move origin to center: (fragCoord - 0.5 * iResolution.xy)
    // 2. Normalize by the SHORTEST side to prevent stretching
    // 3. Zoom is set to 3.0 so you can clearly see the two spiral poles
    float zoom = 2.0;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    for(float y = 0.0; y < 1.0; y += 1.0/samples) {
        for(float x = 0.0; x < 1.0; x += 1.0/samples) {
            ...
            finalCol += computeColor(fragCoord + offset);
        }
    }
